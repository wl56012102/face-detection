...
if __name__ == '__main__':
    ...
    writer = None
    writer = cv2.VideoWriter('output1.avi', fourcc, 15, (640, 480), 1)
    while True:
        ret ,img = camera.read()
        faces = get_face(img)
        for (x, y, w, h) in faces:
            ...
            if(int(b)>75):
                ...
                face_name='unknow'
                cv2.putText(img, face_name+" "+str(b), (x, y - 20), cv2.FONT_HERSHEY_SIMPLEX, 1,(0, 0, 255), 1)
            else:
                for image_path in image_paths:
                    if (os.path.split(image_path)[1].split('.')[0] == str(a)):
                        face_name=os.path.split(image_path)[1].split('.')[1]
                cv2.rectangle(img, (x, y), (x + w, y + h), (255, 0, 0), 2)
                cv2.putText(img,str(a)+" "+face_name+" "+str(int(b)),(x,y-20),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2)
        output = img
        writer.write(output)
